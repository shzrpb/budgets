-- Home page reads only open accounts, but still needs each closed account's
-- closure date so the net worth series knows when to stop carrying its
-- balance forward. Safe to re-run.
